...
export default function DeviceEdit({ params }: { params: { id: string } }) {
  const { id } = params;
  const [name, setName] = useState(''); // TODO: Buscar nome do dispositivo pela API
  const [showConfirm, setShowConfirm] = useState(false);
  const router = useRouter();

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    // TODO: Chamar API para editar dispositivo
    router.push('/device');
  };

  const handleDelete = async () => {
    // TODO: Chamar API para deletar dispositivo
  ...
  return (
    <main className="min-h-screen flex flex-col items-center bg-white p-8">
      <div className="w-full max-w-md bg-gray-100 rounded-2xl shadow-lg p-8 flex flex-col items-center">
        <h2 className="text-3xl font-bold mb-6">Editar Dispositivo #{id}</h2>
        ...
        <button onClick={() => setShowConfirm(true)} className="mt-6 text-red-600 font-semibold">Excluir Dispositivo</button>
        {showConfirm && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
            <div className="bg-white p-8 rounded shadow-lg flex flex-col items-center">
              <p className="mb-4">Tem certeza que deseja excluir este dispositivo?</p>
