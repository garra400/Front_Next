...
export default function PropertyEdit({ params }: { params: { id: string } }) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    // TODO: Chamar API para editar propriedade
    router.push('/property');
  };

  const handleDelete = async () => {
    // TODO: Chamar API para deletar propriedade
  ...
  return (
    <main className="min-h-screen flex flex-col items-center bg-white p-8">
      <div className="w-full max-w-md bg-gray-100 rounded-2xl shadow-lg p-8 flex flex-col items-center">
        ...
        <button onClick={() => setShowConfirm(true)} className="mt-6 text-red-600 font-semibold">Excluir Propriedade</button>
        {showConfirm && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
            <div className="bg-white p-8 rounded shadow-lg flex flex-col items-center">
              <p className="mb-4">Tem certeza que deseja excluir esta propriedade?</p>
