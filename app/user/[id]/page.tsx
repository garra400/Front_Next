...
export default function UserEdit({ params }: { params: { id: string } }) {
  const [name, setName] = useState(''); // TODO: Buscar nome do usuário pela API
  const [showConfirm, setShowConfirm] = useState(false);
  const router = useRouter();

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    // TODO: Chamar API para editar usuário
    router.push('/user');
  };

  const handleDelete = async () => {
    // TODO: Chamar API para deletar usuário
  ...
  return (
    <main className="min-h-screen flex flex-col items-center bg-white p-8">
      <div className="w-full max-w-md bg-gray-100 rounded-2xl shadow-lg p-8 flex flex-col items-center">
        <h2 className="text-3xl font-bold mb-6">Editar Usuário</h2>
        ...
        <button onClick={() => setShowConfirm(true)} className="mt-6 text-red-600 font-semibold">Excluir Usuário</button>
        {showConfirm && (
