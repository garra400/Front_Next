...
export default function SensorEdit({ params }: { params: { id: string } }) {
  const { id } = params;
  const [name, setName] = useState(''); // TODO: Buscar nome do sensor pela API
  const [showConfirm, setShowConfirm] = useState(false);
  const router = useRouter();

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    // TODO: Chamar API para editar sensor
    router.push('/sensor');
  };

  const handleDelete = async () => {
    // TODO: Chamar API para deletar sensor
  ...
  return (
    <main className="min-h-screen flex flex-col items-center bg-white p-8">
      <div className="w-full max-w-md bg-gray-100 rounded-2xl shadow-lg p-8 flex flex-col items-center">
        <h2 className="text-3xl font-bold mb-6">Editar Sensor #{id}</h2>
        ...
        <button onClick={() => setShowConfirm(true)} className="mt-6 text-red-600 font-semibold">Excluir Sensor</button>
        {showConfirm && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
            <div className="bg-white p-8 rounded shadow-lg flex flex-col items-center">
              <p className="mb-4">Tem certeza que deseja excluir este sensor?</p>
